...
from image_fetcher import search_image_google, search_image_pexels
...
async def post_to_telegram():
    ...
    image_url = search_image_google(f"{query} site:pinterest.com")

    if not image_url:
        print("🔍 Google не нашёл изображение, пробуем Pexels...")
        image_url = search_image_pexels(query)

    if not image_url:
        print("⚠️ Ни один источник не дал изображения!")
    ...
    try:
        if image_url and image_url.startswith("http"):
            ...
            print(f"⚠️ Изображение не найдено или некорректное: {image_url}")
            await bot.send_message(chat_id=CHAT_ID, text=title)
            log_info(f"📝 Отправлен заголовок без фото: {popular_topic}")
        ...
        try:
            if image_url and image_url.startswith("http"):
                print(f"📤 Повторная отправка изображения: {image_url}")
                await bot.send_photo(chat_id=CHAT_ID, photo=image_url, caption=title)
            else:
                await bot.send_message(chat_id=CHAT_ID, text=title)
